...
import { postsData, getCommentIdCounter } from '../data/postsData';

function AnnouncementDetail() {
  const { id } = useParams();
  const navigate = useNavigate();
  const postId = parseInt(id) || 1;
  const [post, setPost] = useState(postsData.announcements[postId] || postsData.announcements[1]);
  ...
  return (
    <>
      ...
      <div className="container">
        <Link to="/announcements" style={{color: 'var(--accent)', textDecoration: 'none', marginBottom: '1rem', display: 'inline-block'}}>← 목록으로 돌아가기</Link>
        ...
        <div className="post-detail">
          <div className="detail-header">
            ...
            <div className="detail-actions">
              <Link to={`/announcement-edit/${postId}`} className="btn btn-secondary" style={{textDecoration: 'none'}}>수정</Link>
...
function CommentItem({ comment, postId, isAnnouncement, onUpdate }) {
